(function($) {
    $.fn.soslider = function(options) {
        ...
        if (opts.debug) {
            debug(this, "Plugin initiate: orientation " + opts.orientation + ", middle = " + opts.middle + ", top = " + opts.top + ", left = " + opts.left + ",border_width = " + opts.border_width + ", border_color = " + opts.border_color);
        ...
        if (opts.orientation == "left" || opts.orientation == "right") $content.css("width", opts.width - opts.border_width + "px");
        else $content.css("width", opts.width - opts.border_width * 2 + "px");
        if (opts.orientation == "top" || opts.orientation == "bottom") $content.css("height", opts.height - opts.border_width + "px");
        else $content.css("height", opts.height - opts.border_width * 2 + "px");
        ...
        var eventHandlers = {
            ...
            mouseleave: function(obje, ev) {
                if (!opts.dock) {
                    ...
                    if (opts.gpcheck && undefined !== ev && undefined !== ev.target.title && undefined !== ev.target.src && ev.target.src.indexOf("apis.google.com") > 0) {
                        return
                    }
                    opts.slide_speed = parseInt(opts.slide_speed);
                    switch (opts.orientation) {
                        case "right":
                            ...
                            $(obje).stop().animate({
                                right: r_value
                            }, opts.slide_speed, "linear", function() {
                                changeZIndex(obje, opts, -1)
                            });
                            break;
                        case "left":
